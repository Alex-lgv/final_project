...
class Output extends React.Component {
  ...
  parseXml(xmlText) {
    ...
    return <p className={styles.text__news}>{sentence[0].value}</p>;
  }
  ...
  mapBlockInfo() {
    ...
    return <div className={styles.block__info}>{mapInfo}</div>;
  }
    ...
    const mapDoc = this.state.publicDoc.map((doc) => (
      <div className={styles.column}>
        <div className={styles.content__flex}>
          <p className={styles.data__doc}>
            {this.formatterDate(doc.ok.issueDate)}
          </p>
        ...
        <div>
          <button className={styles.button__source}>
            <Link to={doc.ok.url}>Читать в источнике</Link>
          </button>
          <p className={styles.counter}>{doc.ok.attributes.wordCount} слова</p>
        </div>
      </div>
    ));
    return <div className={styles.documents__content}>{mapDoc}</div>;
  }
